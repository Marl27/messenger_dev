...
def write_private_chat(conn, cursor, sender_id, receiver_id, message, is_broadcasted=False, group_name=None,
                       is_stared=False):
...
def write_group_chat(conn, cursor, sender_id, receiver_id, message, is_broadcasted=False, group_name=None,
                     is_stared=False):
...
def write_chat(conn, cursor, sender_id, receiver_id, message, is_broadcasted=False, group_name=None, is_stared=False):
    receiver = list(map(int, receiver_id.split(',')))
    if len(receiver) > 1:
        logging.info('Writing Group Chat')
        return write_group_chat(conn, cursor, sender_id, receiver_id, message, is_broadcasted, group_name, is_stared)
    else:
        logging.info('Writing Private Chat')
        return write_private_chat(conn, cursor, sender_id, receiver[0], message, is_broadcasted, group_name, is_stared)
